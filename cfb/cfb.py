...
class cfb:
    ...
    def get_records(self,df,confs):
        records_inc = pd.DataFrame(columns=['team', 'opp', 'home_wins', 'away_wins', 'points_against', 'points_for',
       'home_win_pct', 'total_wins', 'total', 'away_win_pct', 'win_pct',
       'home_points', 'away_points', 'home_conference', 'home_conf_label',
       'away_conference', 'away_conf_label', 'home_win', 'away_win', 'spread',
       'week' ])
        for wks in range(2,15,1):
            res = ps.sqldf("""
                with matches as (
                    select home_team, away_team,home_points, away_points,home_conference,c.conf_label home_conf_label
                    ,away_conference,c2.conf_label away_conf_label
                    ...
                    ,home_points - away_points  as spread
                    ,week
                    from df 
                    inner join confs c on df.home_conference = c.conference
                    inner join confs c2 on df.away_conference = c2.conference
                    where week = '""" + str(wks) + """'
                ),home_win_base as
                (
                    select home_team, case when home_points > away_points then 1 else 0 end as home_win
                    ,case when home_points < away_points then 1 else 0 end as home_loss
                    , home_points points_for, away_points points_against
                    from df 
                    where week < '""" + str(wks) + """'
                ),away_win_base as
                (
                    select away_team, case when home_points < away_points then 1 else 0 end as away_win
                    ,case when home_points > away_points then 1 else 0 end as away_loss
                    , home_points points_against, away_points points_for
                    from df 
                    where week < '""" + str(wks) + """'
                ),home_wins as(
                    select home_team, sum(home_win) home_wins
                    ,sum(points_against) points_against, sum(points_for) points_for
                    ,sum(home_loss) home_losses
                    from home_win_base 
                    group by home_team
                ),away_wins as (
                    select away_team, sum(away_win) away_wins
                    ,sum(away_loss) away_losses
                    ,sum(points_against) points_against, sum(points_for) points_for
                    from away_win_base 
                    group by away_team
                )
                select h.home_team team, m.away_team opp, home_wins, away_wins
                , (h.points_against+a.points_against) points_against
                ,( h.points_for+ a.points_for ) points_for
                ...
                ,home_points,away_points, home_conference,  home_conf_label,away_conference
                ,away_conf_label
                ,home_win,away_win
                ,spread
                ,week
                from home_wins h left join away_wins a 
                on h.home_team = a.away_team
                inner join matches m
                on m.home_team = h.home_team
                --where m.home_team = 'Mississippi State'
    ...
    # week one
        ...
        --,opp_win_pct opp_win_pct_week_minus_one
        --,opp_home_win_pct opp_home_win_pct_week_minus_one
        --,opp_away_win_pct opp_away_win_pct_week_minus_one
        ,home_wins home_wins_week_minus_one
        ,away_wins away_wins_week_minus_one
        ,points_against points_against_week_minus_one
        ,points_for points_for_week_minus_one
        ,total_wins total_wins_week_minus_one
        ,total total_week_minus_one
        ,home_points home_points_week_minus_one
        ,away_points away_points_week_minus_one
        ,home_win home_win_week_minus_one
        ,away_win away_win_week_minus_one
        from  records  r   
        ...
        ,home_wins home_wins_week_minus_two
        ,away_wins away_wins_week_minus_two
        ,points_against points_against_week_minus_two
        ,points_for points_for_week_minus_two
        ,total_wins total_wins_week_minus_two
        ,total total_week_minus_two
        ,home_points home_points_week_minus_two
        ,away_points away_points_week_minus_two
        ,home_win home_win_week_minus_two
        ,away_win away_win_week_minus_two
        from  records  r   
        ...
        week_minus_three_home = ps.sqldf("""     
            ...
            ,home_wins home_wins_week_minus_three
            ,away_wins away_wins_week_minus_three
            ,points_against points_against_week_minus_three
            ,points_for points_for_week_minus_three
            ,total_wins total_wins_week_minus_three
            ,total total_week_minus_three
            ,home_points home_points_week_minus_three
            ,away_points away_points_week_minus_three
            ,home_win home_win_week_minus_three
            ,away_win away_win_week_minus_three
            from  records  r   
        ...
        week_minus_four_home = ps.sqldf("""     
            ...
            ,home_wins home_wins_week_minus_four
            ,away_wins away_wins_week_minus_four
            ,points_against points_against_week_minus_four
            ,points_for points_for_week_minus_four
            ,total_wins total_wins_week_minus_four
            ,total total_week_minus_four
            ,home_points home_points_week_minus_four
            ,away_points away_points_week_minus_four
            ,home_win home_win_week_minus_four
            ,away_win away_win_week_minus_four
            from  records  r   
        ...
        week_minus_five_home = ps.sqldf("""     
            ...
            ,home_wins home_wins_week_minus_five
            ,away_wins away_wins_week_minus_five
            ,points_against points_against_week_minus_five
            ,points_for points_for_week_minus_five
            ,total_wins total_wins_week_minus_five
            ,total total_week_minus_five
            ,home_points home_points_week_minus_five
            ,away_points away_points_week_minus_five
            ,home_win home_win_week_minus_five
            ,away_win away_win_week_minus_five
            from  records  r   
        ...
        df_train = ps.sqldf("""  
            select m.team, m.opp, target, spread, m.team_label, m.opp_label
                ...
                ,coalesce(home_wins_week_minus_one,0) home_wins_week_minus_one
                ,coalesce(away_wins_week_minus_one,0) away_wins_week_minus_one
                ,coalesce(points_against_week_minus_one,0) points_against_week_minus_one
                ,coalesce(points_for_week_minus_one,0) points_for_week_minus_one
                ,coalesce(total_wins_week_minus_one,0) total_wins_week_minus_one
                ,coalesce(total_week_minus_one,0) total_week_minus_one
                ,coalesce(home_points_week_minus_one,0) home_points_week_minus_one
                ,coalesce(away_points_week_minus_one,0) away_points_week_minus_one
                ,coalesce(home_win_week_minus_one,0) home_win_week_minus_one
                ,coalesce(away_win_week_minus_one,0) away_win_week_minus_one
                ...
                ,coalesce(home_wins_week_minus_two,0) home_wins_week_minus_two
                ,coalesce(away_wins_week_minus_two,0) away_wins_week_minus_two
                ,coalesce(points_against_week_minus_two,0) points_against_week_minus_two
                ,coalesce(points_for_week_minus_two,0) points_for_week_minus_two
                ,coalesce(total_wins_week_minus_two,0) total_wins_week_minus_two
                ,coalesce(total_week_minus_two,0) total_week_minus_two
                ,coalesce(home_points_week_minus_two,0) home_points_week_minus_two
                ,coalesce(away_points_week_minus_two,0) away_points_week_minus_two
                ,coalesce(home_win_week_minus_two,0) home_win_week_minus_two
                ,coalesce(away_win_week_minus_two,0) away_win_week_minus_two
                --
                ...
                ,coalesce(home_wins_week_minus_four,0) home_wins_week_minus_four
                ,coalesce(away_wins_week_minus_four,0) away_wins_week_minus_four
                ,coalesce(points_against_week_minus_four,0) points_against_week_minus_four
                ,coalesce(points_for_week_minus_four,0) points_for_week_minus_four
                ,coalesce(total_wins_week_minus_four,0) total_wins_week_minus_four
                ,coalesce(total_week_minus_four,0) total_week_minus_four
                ,coalesce(home_points_week_minus_four,0) home_points_week_minus_four
                ,coalesce(away_points_week_minus_four,0) away_points_week_minus_four
                ,coalesce(home_win_week_minus_four,0) home_win_week_minus_four
                ,coalesce(away_win_week_minus_four,0) away_win_week_minus_four
                ...
                ,coalesce(home_wins_week_minus_five,0) home_wins_week_minus_five
                ,coalesce(away_wins_week_minus_five,0) away_wins_week_minus_five
                ,coalesce(points_against_week_minus_five,0) points_against_week_minus_five
                ,coalesce(points_for_week_minus_five,0) points_for_week_minus_five
                ,coalesce(total_wins_week_minus_five,0) total_wins_week_minus_five
                ,coalesce(total_week_minus_five,0) total_week_minus_five
                ,coalesce(home_points_week_minus_five,0) home_points_week_minus_five
                ,coalesce(away_points_week_minus_five,0) away_points_week_minus_five
                ,coalesce(home_win_week_minus_five,0) home_win_week_minus_five
                ,coalesce(away_win_week_minus_five,0) away_win_week_minus_five
